	...
	class TGM_Plugin_Activation {
		...
		public function __construct() {
			...
			$this->strings = array(
				...
				'installing'                      => __( 'Installing Plugin: %s', 'reboot' ),

				'oops'                            => __( 'Something went wrong.', 'reboot' ),
				...
				'complete'                        => __( 'All plugins installed and activated successfully. %1$s', 'reboot' ),
		...
		public function install_plugins_page() {
			...
			<div class="tgmpa wrap">
				...
				<?php if ( isset( $this->message ) ) _e( wp_kses_post( $this->message ), 'reboot' ); ?>
			...
			if ( isset( $_GET[sanitize_key( 'plugin' )] ) && ( isset( $_GET[sanitize_key( 'tgmpa-install' )] ) && 'install-plugin' == $_GET[sanitize_key( 'tgmpa-install' )] ) ) {
				...
				if ( $this->is_automatic ) {
					...
					if ( is_wp_error( $activate ) ) {
						...
						echo '<p><a href="' . add_query_arg( 'page', $this->menu, admin_url( $this->parent_url_slug ) ) . '" title="' . esc_attr( $this->strings['return'] ) . '" target="_parent">' . __( 'Return to Required Plugins Installer', 'reboot' ) . '</a></p>';
				...
				if ( empty( $complete ) ) {
					...
					echo '<p>' .  sprintf( $this->strings['complete'], '<a href="' . admin_url() . '" title="' . __( 'Return to the Dashboard', 'reboot' ) . '">' . __( 'Return to the Dashboard', 'reboot' ) . '</a>' ) . '</p>';
			...
			elseif ( isset( $_GET[sanitize_key( 'plugin' )] ) && ( isset( $_GET[sanitize_key( 'tgmpa-activate' )] ) && 'activate-plugin' == $_GET[sanitize_key( 'tgmpa-activate' )] ) ) {
				...
				if ( is_wp_error( $activate ) ) {
					...
					if ( ! isset( $_POST[sanitize_key( 'action' )] ) ) {
						...
						$msg = sprintf( __( 'The following plugin was activated successfully: %s.', 'reboot' ), '<strong>' . $plugin['name'] . '</strong>' );
				...
				if ( ! empty( $message ) ) {
					...
					$action_links = apply_filters(
						...
						array(
							...
							'dismiss'  => '<a class="dismiss-notice" href="' . add_query_arg( 'tgmpa-dismiss', 'dismiss_admin_notices' ) . '" target="_parent">' . __( 'Dismiss this notice', 'reboot' ) . '</a>',
...
if ( ! class_exists( 'TGMPA_List_Table' ) ) {
	...
	class TGMPA_List_Table extends WP_List_Table {
		...
		protected function _gather_plugin_data() {
			...
			foreach ( TGM_Plugin_Activation::$instance->plugins as $plugin ) {
				...
				if ( isset( $plugin['external_url'] ) ) {
					...
					$table_data[$i]['source'] = __( 'External Link', 'reboot' );
				...
				elseif ( isset( $plugin['source'] ) ) {
					...
					if ( preg_match( '|^http(s)?://|', $plugin['source'] ) )
						...
						$table_data[$i]['source'] = __( 'Private Repository', 'reboot' );

					/** The plugin is pre-packaged with the theme */

					else

						$table_data[$i]['source'] = __( 'Pre-Packaged', 'reboot' );
				...
				/** The plugin is from the WordPress repository */
					...
					$table_data[$i]['source'] = __( 'WordPress Repository', 'reboot' );

				}



				$table_data[$i]['type'] = $plugin['required'] ? __( 'Required', 'reboot' ) : __( 'Recommended', 'reboot' );



				if ( ! isset( $installed_plugins[$plugin['file_path']] ) )

					$table_data[$i]['status'] = sprintf( '%1$s', __( 'Not Installed', 'reboot' ) );

				elseif ( is_plugin_inactive( $plugin['file_path'] ) )

					$table_data[$i]['status'] = sprintf( '%1$s', __( 'Installed But Not Activated', 'reboot' ) );
		...
		public function no_items() {
			...
			printf( __( 'No plugins to install or activate. <a href="%1$s" title="Return to the Dashboard">Return to the Dashboard</a>', 'reboot' ), admin_url() );
		...
		public function get_columns() {
			...
			$columns = array(
				...
				'plugin' => __( 'Plugin', 'reboot' ),

				'source' => __( 'Source', 'reboot' ),

				'type'   => __( 'Type', 'reboot' ),

				'status' => __( 'Status', 'reboot' )
		...
		public function get_bulk_actions() {
			...
			$actions = array(
				...
				'tgmpa-bulk-install'  => __( 'Install', 'reboot' ),

				'tgmpa-bulk-activate' => __( 'Activate', 'reboot' ),
	...
	if ( ! class_exists( 'TGM_Bulk_Installer' ) ) {
		...
		class TGM_Bulk_Installer extends WP_Upgrader {
			...
			public function install_strings() {
				...
				$this->strings['no_package']          = __( 'Install package not available.', 'reboot' );

				$this->strings['downloading_package'] = __( 'Downloading install package from <span class="code">%s</span>&#8230;', 'reboot' );

				$this->strings['unpack_package']      = __( 'Unpacking the package&#8230;', 'reboot' );

				$this->strings['installing_package']  = __( 'Installing the plugin&#8230;', 'reboot' );

				$this->strings['process_failed']      = __( 'Plugin install failed.', 'reboot' );

				$this->strings['process_success']     = __( 'Plugin installed successfully.', 'reboot' );
			...
			public function activate_strings() {
				...
				$this->strings['activation_failed']  = __( 'Plugin activation failed.', 'reboot' );

				$this->strings['activation_success'] = __( 'Plugin activated successfully.', 'reboot' );
		...
		class TGM_Bulk_Installer_Skin extends Bulk_Upgrader_Skin {
			...
			public function add_strings() {
				...
				if ( TGM_Plugin_Activation::$instance->is_automatic ) {
					...
					$this->upgrader->strings['skin_upgrade_start']        = __( 'The installation and activation process is starting. This process may take a while on some hosts, so please be patient.', 'reboot' );

					$this->upgrader->strings['skin_update_successful']    = __( '%1$s installed and activated successfully.', 'reboot' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', 'reboot' ) . '</span><span class="hidden">' . __( 'Hide Details', 'reboot' ) . '</span>.</a>';

					$this->upgrader->strings['skin_upgrade_end']          = __( 'All installations and activations have been completed.', 'reboot' );

					$this->upgrader->strings['skin_before_update_header'] = __( 'Installing and Activating Plugin %1$s (%2$d/%3$d)', 'reboot' );
				...
				/** Default installation strings */
					...
					$this->upgrader->strings['skin_upgrade_start']        = __( 'The installation process is starting. This process may take a while on some hosts, so please be patient.', 'reboot' );

					$this->upgrader->strings['skin_update_failed_error']  = __( 'An error occurred while installing %1$s: <strong>%2$s</strong>.', 'reboot' );

					$this->upgrader->strings['skin_update_failed']        = __( 'The installation of %1$s failed.', 'reboot' );

					$this->upgrader->strings['skin_update_successful']    = __( '%1$s installed successfully.', 'reboot' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', 'reboot' ) . '</span><span class="hidden">' . __( 'Hide Details', 'reboot' ) . '</span>.</a>';

					$this->upgrader->strings['skin_upgrade_end']          = __( 'All installations have been completed.', 'reboot' );

					$this->upgrader->strings['skin_before_update_header'] = __( 'Installing Plugin %1$s (%2$d/%3$d)', 'reboot' );
			...
			public function bulk_footer() {
				...
				if ( empty( $complete ) ) {
					...
					echo '<p>' .  sprintf( TGM_Plugin_Activation::$instance->strings['complete'], '<a href="' . admin_url() . '" title="' . __( 'Return to the Dashboard', 'reboot' ) . '">' . __( 'Return to the Dashboard', 'reboot' ) . '</a>' ) . '</p>';
