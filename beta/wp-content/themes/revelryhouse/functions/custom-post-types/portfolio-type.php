...
function portfolio_register() {
    $labels = array(
        'name' => __('Portfolio', 'reboot'),
        'add_new' => __('Add New', 'reboot'),
        'add_new_item' => __('Add New Portfolio Item', 'reboot'),
        'edit_item' => __('Edit Portfolio Item', 'reboot'),
        'new_item' => __('New Portfolio Item', 'reboot'),
        'view_item' => __('View Portfolio Item', 'reboot'),
        'search_items' => __('Search Portfolio Items', 'reboot'),
...
function portfolio_help_text($contextual_help, $screen_id, $screen) {
    if ('portfolio' == $screen->id) {
        $contextual_help =
        '<h3>' . __('Things to remember when adding a Portfolio item:', 'reboot') . '</h3>' .
        '<ul>' .
        '<li>' . __('Give the item a title. The title will be used as the item\'s headline.', 'reboot') . '</li>' .
        '<li>' . __('You can choose to insert either single images, image slideshow, or video, below.', 'reboot') . '</li>' .
        '<li>' . __('Enter your portfolio item overview into the Visual or HTML area. The text will appear for your project overview.', 'reboot') . '</li>' .
        '<li>' . __('Choose (or first create) a Project Type. You will need to use these to enable the filterable Portfolio option.', 'reboot') . '</li>' .
        '</ul>';
    }
    elseif ('edit-portfolio' == $screen->id) {
        $contextual_help = '<p>' . __('A list of all Portfolio items appear below. To edit an item, click on the items\'s title.', 'reboot') . '</p>';
    }
...
?>
    ...
    <h2><?php _e('Sort Portfolio Items', 'reboot'); ?> <img src="<?php echo home_url(); ?>/wp-admin/images/loading.gif" id="loading-animation" /></h2>
    <p><?php _e('Click, drag, re-order. Repeat as neccessary. Portfolio item at the top will appear first on your page.', 'reboot'); ?></p>
    <ul id="post-list">
