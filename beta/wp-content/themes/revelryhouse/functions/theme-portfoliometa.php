	...
	foreach ($meta_box_portfolio_portfolio_image['fields'] as $field) {
		...
		switch ($field['type']) {
			...
			?>
				...
				jQuery(function($) {
					...
					$('#gt_images_upload').on('click', function(e) {
						...
						var options = {
							title: '<?php _e("Create Featured Gallery", "reboot"); ?>',
							state: 'gallery-edit',
							frame: 'post',
							selection: selection
						};
						...
						if( frame || selection ) {
							options['title'] = '<?php _e("Edit Featured Gallery", "reboot"); ?>';
						}
						...
						frame.menu.get('view').get('gallery-edit').el.innerHTML = '<?php _e("Edit Featured Gallery", "reboot"); ?>';
						...
						function overrideGalleryInsert() {
    						frame.toolbar.get('view').set({
								insert: {
									style: 'primary',
									text: '<?php _e("Save Featured Gallery", "reboot"); ?>',
									...
									click: function() {
										...
										this.el.innerHTML = '<?php _e("Saving...", "reboot"); ?>';
			...
			<?php
				...
				$thumbs_output = '';
				$button_text = ($meta) ? __('Edit Gallery', 'reboot') : $field['std'];
				if( $meta ) {
					$field['std'] = __('Edit Gallery', 'reboot');
					$thumbs = explode(',', $meta);
					$thumbs_output = '';
