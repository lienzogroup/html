...
<div class="cart_totals <?php if ( $woocommerce->customer->has_calculated_shipping() ) echo 'calculated_shipping'; ?>">
	...
	<?php if(trim(strtolower($slug)) != 'confirmation') { ?>
			...
			<?php if ( ! $woocommerce->shipping->enabled || $available_methods || ! $woocommerce->customer->get_shipping_country() || ! $woocommerce->customer->has_calculated_shipping() ) : ?>
				...
				<h2><?php _e('Your Cart', 'woocommerce'); ?></h2>
				<table cellspacing="0" cellpadding="0">
					<tbody>
						...
						<tr class="discount">
							<th><?php _e('discount', 'woocommerce'); ?> <a href="<?php echo add_query_arg('remove_discounts', '2', $woocommerce->cart->get_cart_url()) ?>"><?php _e('[Remove]', 'woocommerce'); ?></a></th>
			...
			<?php
			/*************************************************/
			$nextPage = get_home_url().'/';
			if(trim(strtolower($slug)) == 'cart') {
				$nextPage .= 'checkout';
				$nextPageClass = 'cart';
			}
			else if(trim(strtolower($slug)) == 'checkout') {
				$previousPage = $nextPage.'cart';
				$nextPage = '#';
				$nextPageClass = 'checkout';
			}
			else if(trim(strtolower($slug)) == 'shipping') {
				$previousPage .= 'checkout';
				$nextPage = '#';
				$nextPageClass = 'payment';
			}
			else if(trim(strtolower($slug)) == 'payment') {
				$previousPage = $nextPage.'checkout';
				$nextPage = '#';
				$nextPageClass = 'payment';
			}
			
			/* ADD PREVIOUS BUTTON*/
			if(trim(strtolower($slug)) == 'checkout') { ?>
				<div id="total_link_container">
					<a href="<?php echo $previousPage;?>" class="cart" id="previouspage">PREVIOUS</a>
					<a href="<?php echo $nextPage;?>" class="<?php echo $nextPageClass;?>" id="nextpage"><span>NEXT</span></a>
				</div>
				<style>
					div#total_link_container {
						overflow: hidden;
					}
					div#total_link_container a {
					}
					div#total_link_container a#previouspage {
					...
					div#total_link_container a#nextpage {
						left: 123px;
					...
					div#total_link_container a#nextpage span{
						position: relative;
						top: -15px;
					}
				</style>
			<?php } else {?>
			<a href="<?php echo $nextPage;?>" class="<?php echo $nextPageClass;?>" id="nextpage">NEXT</a>
			<div style="clear:both;"></div>
		<?php }
		} else { ?>
			<h2>Confirmed</h2>
			<p id="msg">Your order should get there in :</p>
			<div id="delivery">
				<span id="days">4</span>
				<span id="to">TO</span>
				<span id="days">6</span> <br />
				<span id="display_day">DAYS</span>
			</div>
		<?php } ?>	
	</div>
</div>
	
	<p id="notyet">Not Quite Ready to Check Out Yet?</p>
	<a class="continue_shopping" href="<?php echo get_home_url().'/collections/';?>">Continue shopping</a>