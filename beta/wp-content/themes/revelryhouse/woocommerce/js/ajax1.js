...
/* USER ACCOUNT LINKS ******************************************/
	...
	var mainContainer = "#user_container #content #sliderContainer";
		/*Check Container*/
		if(jQuery(mainContainer).css("left") == '0px') {
			mainContainer
			nextContainer = mainContainer+' #slider2';
			currentContainer = mainContainer+' #slider1';
			movement = 'left';
		}
		else {
			nextContainer = mainContainer+' #slider1';
			currentContainer = mainContainer+' #slider2';
			movement = 'right';
		}
		/* GET HREF VALUE */
		...
		/* data: {"field": field,"sort":sort}, */
			...
			/* UPDATE CONTENT */
			jQuery(nextContainer).html(data);
			/* SHOW THE CONTAINER*/
			animatePage(mainContainer,movement,currentContainer,nextContainer);
			/* HIDE MODAL*/
