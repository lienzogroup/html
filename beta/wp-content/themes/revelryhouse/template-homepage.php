...
?>
	<script>
		...
		function homeSlider() {
			...
			if(move == true) {
				/* Get Data*/
					var container = "body.home ul.slides";
					var items = jQuery(container+" li").size() - 2;
					var currentLeft = jQuery(container).css("left").split("px");
					currentLeft = currentLeft[0];
					var lastLeft = items * -1100;	
					var nextLeft = currentLeft - 1100;
				

				/* Slide Active LI */
				var active = "slide-"+(nextLeft / -1100);
				if(lastLeft == currentLeft) {
					active = "slide-1";
				}
				jQuery(container+" li.panel").removeClass("activePage");
				jQuery(container+" li.panel."+active).addClass("activePage");
				//jQuery(container+" li.cloned").removeClass("activePage");
				...
				jQuery(container).animate({
					...
					if(lastLeft == currentLeft) {
						jQuery(container).css("left","-1100px");
					}
					setTimeout(homeSlider, 3000);
				});
			} else {
				move = true;
		...
		jQuery(document).ready(function(){		
			...
			jQuery(".arrow.back,.arrow.forward").click(function() {
				move = false;
			});
	            ...
	            <ul class="slides">
	             ...
	             while ($loop->have_posts()) : $loop->the_post(); ?>     
					   <li class="<?php echo 'slide-'.$count;?>">
							...
							<a href="<?php echo get_post_meta($post->ID, 'gt_slide_url', true) ?>" id="home_slider_link">SHOP THE COLLECTION</a>
							<style>
								#home_slider_link {
							...
							<?php the_post_thumbnail('1100x600'); ?>
          ...
          <section id="latest-news" class="span12">
            ...
            <?php } ?>
          	...
          	<div class="row">
      			...
      			$postcount = 0;						
      			$args = array('post_type' => array('magazine'), 'posts_per_page' => $data['latest_select_news']);
      			$loop = new WP_Query($args);
      			while ($loop->have_posts()) : $loop->the_post();
...
<?php 		else : ?>      	
					...
					<article style="display:none;" class="latest-article span3">	
...
<?php endif; ?>
		...
		<div class="hr-line-bg-magazine">	
			<a href="<?php echo site_url(); ?>/magazine"><h2 class="go-to-the-magazine center">GO TO THE MAGAZINE</h2></a>
		</div> 
